document.addEventListener("DOMContentLoaded", () => {
  // 현재 구조에서는 TOKEN은 쓰이지 않으므로 제거 가능 (fetch 안 함)
  ...
  function show(year, month) {
    panes.forEach((p) => {
      const ok =
        p.dataset.year === String(year) &&
        p.dataset.month === String(month);
      p.classList.toggle("active", ok);
    });
    monthButtons.forEach((b) =>
      b.classList.toggle("active", b.dataset.monthBtn === String(month))
    );
