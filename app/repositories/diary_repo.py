...
# 일기 생성
async def create_diary(user: User, title: str, content: str) -> Post:
...
async def get_diary(post_id: int, user: User) -> Optional[Post]:
    return await Post.filter(id=post_id, author=user).first()
...
async def get_diaries(user: User, month: Optional[int] = None, year: Optional[int] = None, week: Optional[int] = None) -> List[Post]:
    ...
    return await query.order_by("-date").all()

# 일기 수정
async def update_diary(post: Post, title: Optional[str] = None, content: Optional[str] = None) -> Post:
